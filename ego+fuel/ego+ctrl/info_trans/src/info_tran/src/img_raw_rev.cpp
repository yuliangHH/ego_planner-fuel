...
//#define SERVER_IP "127.0.0.1"     // 发送方IP
 #define SERVER_IP "192.168.168.100"     // 发送方IP
...
int ImageRev::run()
{
    ...
    timestamp = (static_cast<uint64_t>(head[15]) << 56) | (static_cast<uint64_t>(head[14]) << 48) | (static_cast<uint64_t>(head[13]) << 40) | (static_cast<uint64_t>(head[12]) << 32) | (static_cast<uint64_t>(head[11]) << 24) | (static_cast<uint64_t>(head[10]) << 16) | (static_cast<uint64_t>(head[9]) << 8) | static_cast<uint64_t>(head[8]);
    //std::cout << "dddd\n";
...
int main(int argc, char **argv) {
...

    {
        ...
        // ros::Time now1 = ros::Time::now();
            ...
            double timestamp_sec = imgrev1.timestamp*1e-6;

            //cout << "timestamp:" << timestamp.sec << ' ' << timestamp.nsec << '\n';

            ros::Time time_now ;
            time_now.fromNSec(timestamp_sec); 
	    ...
	    // cout<<"delay="<<now2.toSec()-now1.toSec()<<endl;
            if (cv::waitKey(1) == 27) {
                break;
            }
            count++;
