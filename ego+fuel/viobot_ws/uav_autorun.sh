...
##reboot
gnome-terminal -t "reboot" -x bash -c "source /home/${user_name}/${code_path}/devel/setup.bash; rostopic pub -1 /viobot/stereo2_ctrl /system_ctrl/algo_ctrl '{header: {seq: 0, stamp: {secs: 0, nsecs: 0}, frame_id: ''}, algo_enable: true, algo_reboot: true, algo_reset: false}'; exec bash -i"
sleep 4
echo "viobot ready."
...
gnome-terminal -t "localposision" -x bash -c "rostopic echo /mavros/local_position/odom; exec bash -i"
sleep 5
#gnome-terminal -t "localposision1" -x bash -c "cd ~/bag && rosbag record -a -x "(/camera/aligned_depth_to_color/image_raw|/camera/aligned_depth_to_color/image_raw/compressed|/camera/aligned_depth_to_color/image_raw/compressed/parameter_descriptions|/camera/aligned_depth_to_color/image_raw/compressed/parameter_updates|/camera/aligned_depth_to_color/image_raw/compressedDepth|/camera/aligned_depth_to_color/image_raw/compressedDepth/parameter_descriptions|/camera/aligned_depth_to_color/image_raw/compressedDepth/parameter_updates|/camera/aligned_depth_to_color/image_raw/theora|/camera/aligned_depth_to_color/image_raw/theora/parameter_descriptions|/camera/aligned_depth_to_color/image_raw/theora/parameter_updates|/viobot/pr_loop/points|/viobot/pr_loop/rdf_points|/viobot/pr_loop/points_adjusted|/camera/depth/image_rect_raw/compressed/parameter_descriptions|/camera/depth/image_rect_raw/compressed/parameter_updates|/camera/depth/image_rect_raw/compressedDepth)"; exec bash -i"
